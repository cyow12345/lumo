...
import { ChatService, ChatMessage, ChatSession, ChatParticipant } from '../services/chatService';
...
const PairChat: React.FC<PairChatProps> = ({ userId, partnerId }) => {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      try {
        setIsLoading(true);
        const chatSession = await ChatService.getOrCreateSession(userId, partnerId);
        ...
        async (payload) => {
          ...
          const formattedMessage: ChatMessage = {
            ...
            sender: {
              ...
              role: session.participants.find(p => p.id === newMessage.sender_id)?.role || 'partner_1',
          ...
          <div className="sticky sm:relative top-0 bg-white p-4 border-b border-lavender/10 sm:border-gray-100">
            <div className="flex items-center gap-2 sm:block">
              ...
              <p className="hidden sm:block text-sm text-gray-500">
                Sprecht offen über eure Beziehung
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((message) => (
              ...
              >
                ...
                >
                  {/* Sender Name (nur bei Partner oder Lumo) */}
                  {message.sender.id !== userId && (
                    <div className="font-medium text-sm mb-1">
                      {message.role === 'assistant' ? 'Lumo' : message.sender.name}
                    </div>
                  )}
